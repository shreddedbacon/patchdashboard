...
$patch_list_link = new PDO('mysql:host='.DB_HOST.';dbname='.DB_NAME.';charset=utf-8', ''.DB_USER.'', ''.DB_PASS.'');
#mysql_select_db(DB_NAME, $patch_list_link);
$patch_list_res = $patch_list_link->query($patch_list_sql);
$patch_list_row = $patch_list_res->fetchAll(PDO::FETCH_ASSOC);
#$patch_list_res = mysql_query($patch_list_sql);
#$patch_list_row = mysql_fetch_array($patch_list_res);
$patches_to_apply_count = $patch_list_row[0]['total_found'];
$patch_list_link = NULL;
$data = "";
...
<?php echo $all_messages_to_send;
unset($all_messages_to_send); ?>
    <div class="row">
